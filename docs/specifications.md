...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│     Ollama      │ ← ローカルLLM (qwen2.5-coder:7b)
└────────┬────────┘
         │
         ▼
    コード生成
...
| **LLM** | Ollama | 0.1.20+ | qwen2.5-coder:7b |
...
**説明**: OpenHandsフレームワークを使用し、ローカルOllama（qwen2.5-coder:7b）でコードを生成する。
...
Resolves #123
Generated by CodingWorker (Ollama qwen2.5-coder:7b)
```
...
- 同時実行は1タスクのみ
- モデルのコンテキスト長制限（qwen2.5-coder:7b は32K tokens）
...
| **qwen2.5-coder:7b** | Alibabaが開発したコーディング特化型LLM（70億パラメータ） |
