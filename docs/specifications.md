...
| **IaC** | Terraform | 1.6+ | State管理: S3 + DynamoDB lock |
| **CI/CD** | GitHub Actions | - | OIDC認証 |
| **ワーカー** | Go | 1.21+ | AWS SDK v2 |
| **エージェント** | Aider | Latest | CLI (Docker不要) |
...
- [ ] 特定のラベル `ai-task` が付与されたIssueのみを対象とする
...
2. メッセージをパース
3. リポジトリクローン・ブランチ作成
4. Aider CLI でコード生成（2パス実行）
5. 検証ループ（build/lint/test）
6. GitHub へ Push & PR作成
7. SQS メッセージ削除
```
...
#### FR-004: Aider + Ollama によるコード生成

**説明**: Aiderフレームワークを使用し、ローカルOllama（qwen2.5-coder:1.5b）でコードを生成する。

**要件**:
- [ ] CLI モードで動作（Docker 不要）
- [ ] Ollama エンドポイント: `http://localhost:11434`
- [ ] モデル: `qwen2.5-coder:1.5b`
- [ ] タイムアウト: 600秒（10分/Aider実行）
...
Aider が内部的に Ollama API を呼び出す。直接のインターフェース管理は不要。
...
試行2: 失敗後60秒待機
試行3: 失敗後120秒待機
失敗 : DLQへ移動 + Issueにラベル `ai-task-failed` 付与
```

**ラベル運用**:
- `ai-task`: 処理対象のIssue
- `ai-task-failed`: 処理失敗時に付与
- `ai-task-done`: 処理成功時に付与（`ai-task`は削除）
...
| 用語 | 説明 |
|:---|:---|
| **Aider** | CLI ベースのコーディングエージェント（Docker 不要） |
| **Ollama** | ローカルLLMを実行するためのツール |
| **qwen2.5-coder:1.5b** | Alibabaが開発したコーディング特化型LLM（15億パラメータ） |
